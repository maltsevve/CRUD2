...
import com.maltsevve.crud2.model.builders.writer.ParticularWriterBuilder;
import com.maltsevve.crud2.model.builders.writer.WriterDirector;
...
public class WriterView {
    ...
    private final PostView pv = new PostView(); // Временное решение, чтобы не раздувать код еще сильнее.
    private final WriterDirector writerDirector = new WriterDirector();
        ...
        if (input.matches("\\d+")) {
            switch (Integer.parseInt(input)) {
                case 1 -> { // SAVE
                    ...
                    input = sc.nextLine();
                    writerDirector.setWriterBuilder(new ParticularWriterBuilder(firstName, input, pc.getAll()));
                    wc.save(writerDirector.buildWriter());
                    System.out.println();
                    logic();
                }
                case 2 -> { // UPDATE
                                ...
                                if (str.length == 2 && str[0].matches("\\d+")) {
                                    ...
                                    if (writers.stream().anyMatch(l -> l.getId() == (Long.parseLong(str[0])))) {
                                        ...
                                        if (Integer.parseInt(str[0]) > 0 && !oldWriter.getFirstName().equals(str[1])) {
                                            ...
                                            writerDirector.setWriterBuilder(new ParticularWriterBuilder(str[1],
                                                    lastName, posts));
                                            Writer writer = writerDirector.buildWriter();
                                ...
                                if (str.length == 2 && str[0].matches("\\d+")) {
                                    ...
                                    if (writers.stream().anyMatch(l -> l.getId() == (Long.parseLong(str[0])))) {
                                        ...
                                        if (Integer.parseInt(str[0]) > 0 && !oldWriter.getLastName().equals(str[1])) {
                                            ...
                                            writerDirector.setWriterBuilder(new ParticularWriterBuilder(firstName,
                                                    str[1], posts));
                                            Writer writer = writerDirector.buildWriter();
                                            writer.setId(Long.parseLong(str[0]));
